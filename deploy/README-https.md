...
视频预览（`AVCanvas`）和时间轴缩略图都依赖浏览器的 **WebCodecs API**（`VideoDecoder`）。
该 API 只在「安全上下文」（secure context）下可用：**HTTPS** 或 **localhost**。

当前通过 `http://85.137.247.69:10588` 这种「HTTP + 裸 IP」方式远程访问时，
`window.isSecureContext === false`，`VideoDecoder` 为 `undefined`，
导致视频无法解码 —— 预览区一片黑、缩略图只显示占位图标。
