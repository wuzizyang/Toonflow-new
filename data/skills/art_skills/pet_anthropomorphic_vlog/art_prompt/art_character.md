...
2. **行为拟人、身体保真（情境化）** — 情境化姿态（操作态后腿坐起/立起 + 前爪操作、行动态自然四足不直立行走）、禁人形化修长躯干/宽肩/灵巧人手/挺直军姿/人腿剪影、穿戴宠物专用服饰等全局约束见 `prefix.md#R2` / `prefix.md#R3` / `prefix.md#X6` / `prefix.md#X6b` / `prefix.md#X6c`（运行时由 prefix 注入）。**本角色专属**：基础着装由角色设定决定，后续特定服化为叠加层
3. **四视图一致** — 种属/毛色/体型/服装跨视图高度统一
4. **照片级写实** — 照片级写实质感锚定（单个 `photorealistic` 不堆叠写实同义词、真实毛发清晰自然/皮毛层次/自然透光感、真实解剖结构、绝非 CG/3D 渲染/卡通/插画/黏土）全局规范见 `prefix.md#R1` / `prefix.md#R4` / `prefix.md#S8` / `prefix.md#X1`（运行时由 prefix 注入）
...
| 风格底色 | 照片级写实、真实毛发、自然日常光照、自然透光感、vlog自拍对镜头感（photorealistic, natural ambient light, vlog selfie look）；**写实词不堆叠规范见 `prefix.md#S8`** |
...
> **姿态情境化（核心规则）**：操作态（做需用"手"的事时后腿坐起/立起、前爪操作，含背微弓、重心压后腿、非人类挺直站姿）与行动态（走路/奔跑/移动等位移回到自然四足步态、不直立行走）的姿态切换全局规范见 `prefix.md#R2` / `prefix.md#X6`（运行时由 prefix 注入）。
>
> **本角色专属**：兽态是本体，拟人只是"为了用手做事而临时立起"。**四视图基础设定图以「自然真实宠物（自然坐姿/站姿、真实兽态）」为基准呈现角色**——基础设定图不做"立起+整套人类式服装"的拟人造型（独立全身写实拟人着装形象易被图像审核打回）；**拟人操作态（后腿坐起/立起+前爪操作）与完整服化在分镜/动作镜头按需呈现**（分镜带场景，可正常过审，见 director 技能）。
...
| R10 | 必须指定「photorealistic（**单个写实词，不堆叠 real-life photo/snapshot 等同义词，见 `prefix.md#S8`**）+ 真实毛发清晰自然 + 自然透光感」质感锚定词 |
