...
> 基础服装由角色描述（身份/朝代/职业/场景）决定其最自然的常规着装，作为该角色「日常默认态」；正式礼服/特殊衍生在服化衍生环节叠加。**着装须得体、完整覆盖躯干**。
...
|---|---|
| R1 | 必须为「素颜无妆」状态 |
| R2 | 必须根据角色描述声明合适的常规古装着装作为基础服装（如闺秀→素色长裙、书生→素色长衫、武将→轻便武服）；着装须得体、完整覆盖躯干 |
| R3 | 必须声明「无发饰、无配饰」 |
| R4 | 必须指定「纯净中性灰背景」 |
| R5 | 必须指定「四视图一致性」 |
...
| 编号 | 严禁 |
|---|---|
| X1 | 着装不得体或覆盖不全；与角色描述明显不符的着装；过度复杂花纹/装饰干扰后续服化叠加 |
| X2 | 正顶硬光/正底光/彩色光 |
| X3 | 过度美白至无血色 / 肤色发灰 |
| X4 | 复杂场景背景（必须纯灰底） |
| X5 | 夸张表情/动态姿势 |
